...
export class StatoSessioneClientService {
  // Creo il servizio che mantiene lo stato della sessione nel client
  ...
  constructor() {
    // Eseguo questa logica quando il servizio viene creato
    const rawLocal = localStorage.getItem('auth');
    const rawSession = sessionStorage.getItem('auth');
    const raw = rawLocal ?? rawSession;

    if (raw) {
      // Controllo se esiste qualcosa salvato
      try {
        // Provo a parsare la stringa JSON
        ...
        // Gestisco errori di JSON non valido
        console.warn(
          '[StatoSessioneClientService] errore parse auth da localStorage',
          e,
        );
  ...
  segnaSessioneVerificata(): void {
    // Creo un metodo per segnare che il server ha confermato la sessione
    if (!this.sessioneVerificata$.value) {
      // Evito di settare true più volte se è già verificata
      const now = performance.now(); // Prendo un timestamp per debug prestazioni
      console.log(
        'RISPOSTA SERVER CONFERMA STATO SESSIONE alle ' + now + ' ms',
      ); // Loggo quando arriva la conferma sessione
  ...
  reset(consideraSessioneGiaVerificata: boolean = true): void {
    // Creo un metodo per resettare lo stato della sessione
