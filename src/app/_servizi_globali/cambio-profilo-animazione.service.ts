...
export class CambioProfiloAnimazioneService {
  ...
  async apriProfilo(): Promise<void> {
    ...
    const navigaAProfilo = async () => {
      ...
      const segmento = codice === 'it' ? 'profilo' : 'profile';
    ...
    if (saturnoNascosto || sfondoNascosto) {
      ...
      setTimeout(() => navigaAProfilo(), 1050);
    } else {
      if (scene) {
