...
export class CaroselloGettersUtility {
   ...
   */
  static getPrevTitolo(ctx: any): string {
    // Recupero il titolo della slide precedente (rispetto alla corrente)
   ...
   */
  static getNextTitolo(ctx: any): string {
   ...
   */
  static getIndiceRealeZeroBased(ctx: any): number {
    // Calcolo l'indice reale 0-based della slide corrente (ignorando eventuali cloni)
   ...
   */
  static getTitoloCorrente(ctx: any): string {
    // Recupero il titolo della slide corrente
    if (!ctx.titoliAlt.length) return ''; // Esco con stringa vuota se l'array titoli e' vuoto
    return (
      ctx.titoliAlt[CaroselloGettersUtility.getIndiceRealeZeroBased(ctx)] || ''
    ); // Prendo il titolo all'indice reale corrente o fallback a stringa vuota
  }
   ...
   */
  static getImgTitoloCorrente(ctx: any): string {
    // Recupero l'immagine titolo della slide corrente
    if (!ctx.imgTitolo.length) return ''; // Esco con stringa vuota se l'array immagini titolo e' vuoto
    return (
      ctx.imgTitolo[CaroselloGettersUtility.getIndiceRealeZeroBased(ctx)] || ''
    ); // Prendo l'immagine titolo all'indice reale corrente o fallback a stringa vuota
  }
   ...
   */
  static getSottotitoloCorrente(ctx: any): string {
    // Recupero il sottotitolo della slide corrente
    if (!ctx.sottotitoli.length) return ''; // Esco con stringa vuota se l'array sottotitoli e' vuoto
    return (
      ctx.sottotitoli[CaroselloGettersUtility.getIndiceRealeZeroBased(ctx)] ||
      ''
    ); // Prendo il sottotitolo all'indice reale corrente o fallback a stringa vuota
  }
}
