...
export class CaroselloScrollStateUtility {
   ...
   * @returns void
   */
  static avviaCambioSlideConFade(
    ctx: any,
    azioneScorrimento: () => void,
  ): void {
    // Avvio un cambio slide assicurandomi di fare fade-out audio e stop video
    if (!ctx.player) {
      // se dovessi avere problemi con il player..
    ...
    fade.finally(() => {
      // Dopo il fade-out, fermo e resetto il player
      try {
        ctx.player.pause();
      } catch {}
      try {
        if (
          ctx.player &&
          !ctx.player.isDisposed() &&
          typeof ctx.player.readyState === 'function' &&
          ctx.player.readyState() >= 1
        ) {
   ...
   * @returns void
   */
  static vaiAvantiDaFineTrailer(ctx: any): void {
    ...
    const chiudi = () => {
      ...
      if (ctx.alTop && !ctx.pausaPerScroll) {
        // Se sono al top e non sono in pausa scroll, posso avviare il trailer della nuova slide
    ...
    if (ctx.indiceCorrente > ctx.immagini.length) {
      // Se ho superato l'ultima slide reale, devo fare wrap alla prima
      setTimeout(() => {
        // Aspetto la durata della transizione prima del salto istantaneo
      ...
      setTimeout(() => {
        // Aspetto la fine della transizione prima di chiudere
        chiudi(); // Chiudo e riattivo la logica post-transizione
      }, ctx.durataTransizioneMs); // Sincronizzo con la durata della transizione
    }
  }
   ...
   * @returns void
   */
  static avviaAutoscroll(ctx: any): void {
    // Avvio (o ripianifico) l'autoscroll quando non sono al top
    ...
    ctx.timerAutoscroll = setTimeout(() => {
      // Schedulo il prossimo avanzamento automatico
      ...
      if (ctx.pausaPerHover) return;
      if (ctx.scorrimentoInCorso) {
        // Se sto gia' scorrendo, ripianifico e basta
   ...
   * @returns void
   */
  static fermaAutoscroll(ctx: any): void {
    // Fermo e pulisco il timer dell'autoscroll se presente
    if (ctx.timerAutoscroll) {
      // Controllo se esiste un timeout attivo
  ...
  static leggiIndiceRealeDaStorage(ctx: any): number | null {
    try {
      // Provo a leggere lo storage senza far fallire in ambienti dove non e' disponibile
      const v =
        window && window.localStorage // Controllo che esistano window e localStorage
      ...
      // Se localStorage lancia eccezioni (privacy mode, ecc.)
   ...
   * @returns void
   */
  static salvaIndiceRealeInStorage(ctx: any, indiceReale: number): void {
    // Salvo in localStorage l'indice reale 0-based della slide corrente
    try {
      // Provo a scrivere nello storage senza far fallire in ambienti dove non e' disponibile
      if (window && window.localStorage) {
        // Verifico che esistano window e localStorage
        window.localStorage.setItem(
          ctx.chiaveStorageIndice,
          String(indiceReale),
        ); // Salvo l'indice convertendolo in stringa
      }
