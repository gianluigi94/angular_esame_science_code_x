...
export class CaroselloDatiUtility {
      ...
      novita$.subscribe({
        ...
        next: (elencoOrdinato: NovitaItemMin[]) => {
          ...
          if (ctx.immagini.length > 0) {
            ...
            if (typeof ctx.impostaIndiciSfondiCritici === 'function')
  ...
  /**
   * Costruisce l'URL locale dello sfondo del carosello partendo dalla descrizione semantica.
   * - Pulisce la descrizione ricevuta
   * - Rimuove il prefisso film. oppure serie.
   * - Compone il path finale dello sfondo usando lo slug ottenuto
   *
   * @param descrizione string Descrizione semantica del contenuto.
   * @returns string Percorso locale completo dello sfondo del carosello.
   */
  static urlSfondoDaDescrizione(descrizione: string): string {
    const slug = (descrizione || '')
      .trim() // rimuovo eventuali spazi iniziali e finali dalla descrizione
      .replace(/^film\./i, '') // tolgo il prefisso film. se presente
      .replace(/^serie\./i, ''); // tolgo il prefisso serie. se presente
    return `assets/carosello_locandine/carosello_${slug}.webp`; // costruisco il percorso locale dello sfondo usando lo slug ottenuto
  }

  /**
   * Costruisce l'URL locale dell'immagine titolo partendo dalla descrizione semantica e dalla lingua.
   * - Pulisce la descrizione ricevuta
   * - Rimuove il prefisso film. oppure serie.
   * - Normalizza la lingua in it oppure en
   * - Compone il path finale dell'immagine titolo usando lingua e slug
   *
   * @param descrizione string Descrizione semantica del contenuto.
   * @param lingua string Codice lingua richiesto per l'immagine titolo.
  ...
  static urlTitoloDaDescrizione(descrizione: string, lingua: string): string {
    ...
    const lang = (lingua || 'it').toLowerCase().startsWith('en') ? 'en' : 'it'; // normalizzo la lingua scegliendo en solo se inizia per en, altrimenti uso it
    return `assets/titoli_${lang}/titolo_${lang}_${slug}.webp`; // costruisco il percorso locale dell'immagine titolo usando lingua normalizzata e slug
  }
}
