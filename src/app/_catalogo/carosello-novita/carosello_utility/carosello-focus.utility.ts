...
export class CaroselloFocusUtility {
   ...
   * @returns void
   */
  static gestisciBlurFinestra(ctx: any): void {
    ...
    ctx.sfumaGuadagnoVerso(0, ctx.durataFadeAudioMs).finally(() => {
      // Faccio fade-out audio e poi fermo/resetto il video
      try {
        ctx.player.pause();
      } catch {} // Provo a mettere in pausa senza rompere se il player non e' pronto
      try {
        ctx.player.currentTime(0);
      } catch {} // Provo a riportare il video all'inizio senza rompere in caso di errore
    });
  }
   ...
   * @returns void
   */
  static gestisciFocusFinestra(ctx: any): void {
    // Gestisco cosa succede quando la finestra torna in focus
    if (!ctx.pausaPerBlur) return; // Esco se non ero in pausa per blur
    ctx.pausaPerBlur = false; // Tolgo lo stato di pausa per blur

    if (!ctx.alTop) {
      // Se non sono in cima, riparto l'autoscroll
    ...
    if (ctx.player && !ctx.pausaPerScroll) {
      // Se sono in cima e non sono in pausa per scroll, posso ripartire col trailer
