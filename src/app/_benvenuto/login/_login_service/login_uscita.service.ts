...
export class LoginUscitaService {
   ...
   * Se richiesto, può saltare completamente l'animazione,
   * così un guard di routing può procedere subito al cambio rotta.
   *
   * Comportamento:
   * - Se saltaAnimazioneUscita è true → termina immediatamente.
   * - Altrimenti:
   *    anima l'uscita del contenuto login tramite LoginAnimazioniService
   *    anima il footer  con GSAP
   *    anima il testo footer-p (opacity) con GSAP
  ...
  animaUscita(
    ...
    if (loginContenuto?.nativeElement) {
      // controllo di avere l'elemento del pannello login su cui animare l'uscita
      animazioni.push(
        // aggiungo alla promisea dell'animazione di uscita alla lista
        this.loginAnimazioniService.animaUscita(
          // avvio l'uscita del pannello tramite il servizio specifico
    ...
    if (footer) {
      animazioni.push(
        // aggiungo anche questa animazione alla lista da aspettare
    ...
    if (animazioni.length === 0) {
      // controllo se non ho trovato nulla da animare
