// Servizio che gestisce lo stato di autenticazione dell'utente: lo legge/salva su localStorage e lo espone in modo reattivo tramite BehaviorSubject. Fornisce metodi per aggiornare l'auth globale e per eseguire il logout, resettando anche lo stato della sessione lato client. Si inizia a ragionare anche sul fatto se si è gia verificata la sessione, utile per ui.
...
export class Authservice {
   ...
   */
  leggiObsAuth() {
    // l'observable che permettere agli altri componenti di ascoltare lo stato di autenticazione
   ...
   *
   * @param dati Auth
   *  Nuovo stato di autenticazione da impostare.
   *
   * @returns void
   */
  settaObsAuth(dati: Auth) {
    // aggiorno lo stato di autenticazione in memoria e lo notifico a tutti gli ascoltatori
  ...
  /**
   * Salva (serializza) lo stato di autenticazione nel salvataggio locale (localStorage o sessionStorage),
  ...
  leggiAuthDaStorage(): Auth {
    const tmpLocal: string | null = localStorage.getItem('auth'); //teplate localstorage
    const tmpSession: string | null = sessionStorage.getItem('auth'); //template sessionstorage
    const tmp: string | null = tmpLocal ?? tmpSession;
    let auth: Auth; // dichiaro la variabile che conterrà l'oggetto Auth finale
    if (tmp != null) {
      // controllo se ho trovato qualcosa nel salvataggio locale
  ...
  scriviAuthSuStorage(auth: Auth, restaCollegato: boolean): void {
    // salvo lo stato di autenticazione nello storage corretto
